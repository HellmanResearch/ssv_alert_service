...
#[derive(Serialize, Deserialize)]
pub struct DecidedResponseWrap {
    decided_response: DecidedResponse,
    timestamp: u64,
}
...
async fn main() {
    ...
    loop {
        ...
        let thread = tokio::spawn(async move {
            ...
            loop {
                count += 1;
                // let mut decided_response_vec = vec![];
                let mut decided_buf = "".to_string();
                ...
                if count % 1000 == 0 {
                    match decided_file.write(decided_buf.as_bytes()) {
                        Ok(_) => {}
                        Err(error) => {
                                ...
                                match result {
                                    Ok(decided_response) => {
                                        ...
                                        let timestamp = SystemTime::now()
                                            ...
                                            .unwrap()
                                            .as_secs();
                                        let decided_response_wrap = DecidedResponseWrap{
                                            decided_response: DecidedResponse,
                                            timestamp,
                                        };
                                        let decided_str = serde_json::to_string(&decided_response_wrap).unwrap();
                                        decided_buf.push_str("\n");
                                        decided_buf.push_str(decided_str.as_str());
                                        // decided_response_vec.push(decided_str);
                                    }
                                    Err(error) => {
                                        error!("deserialize decided message error: {}", error);
                                    }
